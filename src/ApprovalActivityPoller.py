...
def send_email(input,task_token,from_email,to_email):
    ...
    <head></head>
    <body>
      <h1>AWS Account Approval Request </h1>
      <p>This email was sent for your approval for the new aws account. <br> {params} <br> Click
        <a href='{approve}'>approve</a> or
    ...
    response = ses_client.send_email(
        ...
        Message={
            'Subject': {
                'Data': 'AWS Account Approval Request',
            },
            'Body': {
                'Html': {
                    'Data': BODY_HTML,
...
def lambda_handler(event, context):
    ...
    approval_activity_arn = os.environ['APPROVAL_ACTIVITY']
    approvar_email = os.environ['APPROVAR_EMAIL']
    ...
    try:
        ...
        logger.debug("Activity response {} ".format(json.dumps(activity_response)))
        ...
        if 'taskToken' in activity_response:
            ...
            send_email(input,task_token,from_email,approvar_email)
